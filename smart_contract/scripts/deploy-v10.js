...
async function main() {
  ...
  console.log("\nDeploying UploadUpgradeableV10 Proxy directly...");
  const UploadV10 = await ethers.getContractFactory("UploadUpgradeableV10");
  const uploadProxy = await upgrades.deployProxy(UploadV10, [], { kind: 'uups' });
  await uploadProxy.waitForDeployment();
  const proxyAddress = await uploadProxy.getAddress();
  console.log("UploadUpgradeableV10 Proxy deployed to:", proxyAddress);
