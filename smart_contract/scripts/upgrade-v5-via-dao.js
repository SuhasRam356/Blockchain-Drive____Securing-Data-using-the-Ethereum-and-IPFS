...
async function main() {
    ...
    // 1. Deploy V5 implementation
    console.log("Deploying V5 Implementation...");
    const UploadV5 = await ethers.getContractFactory("UploadUpgradeableV5");
    const v5Impl = await UploadV5.deploy();
    await v5Impl.waitForDeployment();
    const v5ImplAddress = await v5Impl.getAddress();
    console.log("V5 Implementation deployed to:", v5ImplAddress);

    // 2. Prepare upgrade calldata
    const proxyInterface = new ethers.Interface([
        "function upgradeToAndCall(address newImplementation, bytes data) payable"
    ]);
    const encodedCall = proxyInterface.encodeFunctionData("upgradeToAndCall", [v5ImplAddress, "0x"]);

    // 3. Propose upgrade
    console.log("Creating upgrade proposal...");
    const description = "Upgrade to V5 for File Versioning System";
    ...
    console.log("Upgrade Executed successfully! We are now on V5.");
}
