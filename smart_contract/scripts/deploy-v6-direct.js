...
async function main() {
  ...
  // 3. Deploy V6 Proxy DIRECTLY (bypassing DAO voting for local dev)
  console.log("\nDeploying UploadUpgradeableV6 Proxy directly...");
  const UploadV6 = await ethers.getContractFactory("UploadUpgradeableV6");
  const uploadProxy = await upgrades.deployProxy(UploadV6, [], { kind: 'uups' });
  await uploadProxy.waitForDeployment();
  const proxyAddress = await uploadProxy.getAddress();
  console.log("UploadUpgradeableV6 Proxy deployed to:", proxyAddress);
