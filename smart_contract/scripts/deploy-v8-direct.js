...
async function main() {
  ...
  // Deploy V8 Proxy DIRECTLY (bypassing DAO voting for local dev)
  console.log("\nDeploying UploadUpgradeableV8 Proxy directly...");
  const UploadV8 = await ethers.getContractFactory("UploadUpgradeableV8");
  const uploadProxy = await upgrades.deployProxy(UploadV8, [], { kind: 'uups' });
  await uploadProxy.waitForDeployment();
  const proxyAddress = await uploadProxy.getAddress();
  console.log("UploadUpgradeableV8 Proxy deployed to:", proxyAddress);
