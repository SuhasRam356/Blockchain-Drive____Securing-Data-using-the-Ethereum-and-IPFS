...
async function main() {
    ...
    const UploadV3 = await ethers.getContractFactory("UploadUpgradeableV3");
    const v3Impl = await UploadV3.deploy();
    await v3Impl.waitForDeployment();
    const v3ImplAddress = await v3Impl.getAddress();
    console.log("V3 Implementation deployed to:", v3ImplAddress);

    // 2. Prepare upgrade calldata
    const proxyInterface = new ethers.Interface([
        "function upgradeToAndCall(address newImplementation, bytes data) payable"
    ]);
    const encodedCall = proxyInterface.encodeFunctionData("upgradeToAndCall", [v3ImplAddress, "0x"]);

    // 3. Propose upgrade
    console.log("Creating V3 upgrade proposal...");
    const description = "Upgrade to V3 for Receiver Signatures";
    ...
    console.log("Upgrade Executed successfully! We are now on V3.");
}
