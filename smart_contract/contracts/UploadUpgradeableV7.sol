...
contract UploadUpgradeableV7 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    struct FileInfo {
        string url;
        string category;
    ...
    // --- V7 STORAGE ADDITIONS ---
    // Mapping of file url => receiver address => receiver-encrypted AES key
    mapping(string => mapping(address => string)) public sharedEncryptedAESKeys;
    ...
    // --- V7 SHARING FUNCTIONS ---
    // Used when Alice grants access to Bob for all her existing files
    function shareFileKeysForUser(address user, string[] calldata urls, string[] calldata keys) external {
        require(urls.length == keys.length, "Arrays length mismatch");
        for (uint i = 0; i < urls.length; i++) {
            sharedEncryptedAESKeys[urls[i]][user] = keys[i];
        }
    }

    // Used when Alice uploads a new file, encrypting its key for all users she has already shared with
    function shareFileKeysForMultipleUsers(string calldata url, address[] calldata users, string[] calldata keys) external {
        require(users.length == keys.length, "Arrays length mismatch");
        for (uint i = 0; i < users.length; i++) {
            sharedEncryptedAESKeys[url][users[i]] = keys[i];
        }
    }
}
